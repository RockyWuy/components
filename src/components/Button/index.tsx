...
// 不同类型不同颜色
function typeToColor(type: string = 'default', disabled: boolean = false, style: string): string {
    let colors = {
        primary: {
...
const Button = styled.a<ButtonProps>`
    ...
    padding: 0;
    text-align: center;
    font-size: ${props => props.size === 'small' ? '26px' : props.size === 'large' ? '48px' : '36px'};
    font-weight: 400;
    height: 4.6rem;
    line-height: 4.6rem;

    color: ${props => typeToColor(props.type, props.disabled, 'color')};
    background-color: ${props => typeToColor(props.type, props.disabled, 'bgColor')};
    ...
    &:active {
        background-color: ${props => props.disabled ? '' : activeColor(props.type, 'bgColor')};
        color: ${props => props.disabled ? '' : activeColor(props.type, 'color')};
        border: ${props => props.disabled ? '' : activeColor(props.type, 'border')};
    }
`;
